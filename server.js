...
const axios = require('axios'); // Mantido para futuras implementações
...
app.get('/api/fornecedores', async (req, res) => {
    try {
        const fornecedores = await db.Fornecedor.findAll(); // findAll() retorna todos os atributos por padrão
...
app.post('/api/produtos', async (req, res) => {
    const { nome, descricao, preco_venda, preco_custo, estoque_atual, estoque_minimo, unidade_medida, codigo_barras, ncm, icms_aliquota, origem, ativo, categoria } = req.body;
    ...
    try {
        const novoProduto = await db.Produto.create({
            nome, descricao, preco_venda, preco_custo, estoque_atual, estoque_minimo, unidade_medida, codigo_barras, ncm, icms_aliquota, origem, ativo, categoria
        });
        ...
        const pedidos = await db.PedidoFornecedor.findAll({
            include: [
                ...
                },
                {
                    ...
                    as: 'itens',
                    include: [
                        { model: db.Produto, as: 'produto', attributes: ['id', 'nome', 'unidade_medida', 'preco_venda'] },
